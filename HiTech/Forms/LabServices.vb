...
Public Class labservices
    ...
    Public Sub fill()
        ...
        Try
            copen()
            Dim gt As New SqlCommand("SELECT * FROM labservices ORDER BY id ASC", con)
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        ...
        Try
            copen()
            Dim cmd As New SqlCommand("INSERT INTO labservices (scode,snamear,snameen,dept,sprice) VALUES (@a,@b,@c,@d,@e)", con)
            With cmd.Parameters
            ...
            cmdact = New SqlCommand("INSERT INTO useract (uname,details,acdate) VALUES ('" & un & "', 'قام المستخدم " & un & " بإضافة تركيبة جديدة بالرقم " & scode.Text & "', '" & nowdate() & "')", con)
            cmdact.ExecuteNonQuery()
            cclose()
            reset()
            Cursor = Cursors.Default
            alert("تم إدخال التركيبة بنجاح", 1)
        Catch ex As Exception
            If ex.Message.Contains("duplicate key") Then
                Cursor = Cursors.Default
                alert("كود التركيبة او اسم التركيبة مسجل مسبقاً!", 0)
                Exit Sub
            End If
        End Try
    End Sub
    ...
    Private Sub Button6_Click(sender As Object, e As EventArgs) Handles Button6.Click
        ...
        Try
            copen()
            Dim cmd As New SqlCommand("UPDATE labservices SET scode=@a,snamear=@b,snameen=@c,dept=@d,sprice=@e WHERE id = " & id.Text & "", con)
            With cmd.Parameters
                .Add(New SqlParameter("@a", scode.Text))
                .Add(New SqlParameter("@b", snamear.Text))
                .Add(New SqlParameter("@c", snameen.Text))
            ...
            cmdact = New SqlCommand("INSERT INTO useract (uname,details,acdate) VALUES ('" & un & "', 'قام المستخدم " & un & " بتعديل بيانات التركيبة رقم " & scode.Text & "', '" & nowdate() & "')", con)
            cmdact.ExecuteNonQuery()
            cclose()
            reset()
            Cursor = Cursors.Default
            alert("تم تعديل بيانات التركيبة بنجاح", 1)
        Catch ex As Exception
            If ex.Message.Contains("duplicate key") Then
                Cursor = Cursors.Default
                alert("كود التركيبة او اسم التركيبة مسجل مسبقاً!", 0)
    ...
    Private Sub Button7_Click(sender As Object, e As EventArgs) Handles Button7.Click
        Confirm.table.Text = "labservices"
