...
Public Class Prescrips
    Public Sub search()
        If ser.Text <> "" Then
            ...
            Try
                gt = New SqlCommand("SELECT prescriptions.*, patients.pname FROM prescriptions, patients WHERE prescriptions.pid = patients.pid AND prescriptions.pid = '" & ser.Text & "' AND prescriptions.dstat = 1", con)
    ...
    Public Sub fill()
        ...
        Dim gt As SqlCommand
        Try
            gt = New SqlCommand("SELECT prescriptions.*, patients.pname FROM prescriptions, patients WHERE prescriptions.pid = patients.pid AND prescriptions.dstat = 1", con)
            Dim rdr As SqlDataReader = gt.ExecuteReader
            data.Rows.Clear()
            While rdr.Read
                data.Rows.Add({rdr("did"), rdr("pname"), rdr("doctor"), rdr("ddate"), rdr("dstat")})
